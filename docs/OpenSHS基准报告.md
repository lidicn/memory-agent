...
> 数据完整性：本数据集共有 **13 条 MA 活动记录没有 start_ts/end_ts**（全部是 `bathing`，
> 走占用推断兜底路径），它们无法参与时间定位评估，协议如实计数并剔除，而非默认命中。
...
|---|---|---|---|---|
| bathing | 0.21 | 0.375 | 0.353 | 0.145 |
...
**MA=0.638 vs 最强基线 0.649 ⇒ LIFT −0.011（无增益）**

### Level 2a · 时段级（槽=5 min，3930 槽/活动，13 条记录无区间）
...
| `watching_tv` | F1 0.96「表现最好」 | **虚高，实为最差**：来自 `_tv_from_telemetry` 遥测兜底，区间是**零长度瞬间**；时段级 MCC **−0.099（不如随机）**，段级只召回 29/71 |
...
| `bathing` | F1 0.375 | **无法定位**：13 条记录全部无 `start_ts/end_ts`，时段级/段级均为 0 |

**净结论**：在公平的可比口径下，MA 只有 **sleeping（强）** 和 **cooking（检出强、定位弱）** 有真实信号；
`watching_tv` / `working` / `bathing` 均未通过退化基线的检验或存在结构性缺陷。
...
| 优先级 | 问题 | 改进 |
|---|---|---|
| **P0** | `watching_tv` 遥测兜底产出零长度区间，导致天级虚高、时段级不如随机 | 让其输出真实时间区间；或显式标注「仅日级」并退出时间类评估 |
| **P0** | `bathing` 占用推断兜底不产出 `start_ts/end_ts` | 兜底路径至少输出推断窗口，否则时间定位为零 |
...
改进后重跑同一脚本，以 **LIFT（而非裸 F1）** 判断是否有真实增益。
